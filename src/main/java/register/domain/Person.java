...
@NamedQueries({
        @NamedQuery(name = "Person.findPersons", query = "SELECT p FROM Person p LEFT JOIN FETCH p.passports ps " +
                "LEFT JOIN FETCH p.birthCertificate bs " +
                "WHERE p.personId = :personId")
})
public class Person {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY) // сама БД генерирует Id
